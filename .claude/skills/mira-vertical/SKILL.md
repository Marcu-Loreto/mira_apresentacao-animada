---
name: mira-vertical
description: Gera uma versão VERTICAL (9:16, ex. 640x1080 numa tela 1080p) de um deck do Mira a partir do 16:9 original, OU cria slides verticais DO ZERO na geometria nativa quando não há deck de origem, para vídeo vertical (Reels, Shorts, TikTok, Stories). Na conversão não toca no arquivo original: cria um novo index-9x16.html ao lado. Cada slide de conteúdo fica com SÓ o título no topo e a animação num canvas alto e padronizado abaixo; subtítulo, header do card e a base saem; o título encolhe até caber em 2 linhas; o viewBox da animação é esticado em altura. O eixo de cada animação é reformulado para o retrato (fluxo horizontal vira vertical, comparação lado a lado vira empilhada). Texto, cores, timings e loop ficam intactos. Use SEMPRE que o usuário disser "/mira-vertical", "versão vertical", "deixa vertical", "formato 9:16", "1080x1920", "para Reels", "para Shorts", "para Stories", "para TikTok", "vídeo vertical", "modo retrato", "cria um slide vertical", ou pedir o deck ou um slide novo em formato vertical.
---
