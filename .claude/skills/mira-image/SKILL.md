---
name: mira-image
description: Coloca uma imagem que o usuário JÁ TEM (arquivo local ou URL) dentro de um slide do Mira, novo ou existente, num card limpo onde a imagem fica grande e bem enquadrada. Copia a imagem para a pasta assets/ do deck e referencia por caminho relativo, então o slide continua autossuficiente e abre direto por file:// sem servidor. Card limpo no padrão do mira-3d e do mira-qrcode: só o título do slide (sem ícone, máximo 6 palavras) e a imagem maximizada, sem legenda. A imagem fica estática; o loop interno da Regra Zero vai na moldura (brilho respirando), nunca distorcendo a imagem. NÃO confunda: para GERAR imagem nova use mira-visuals ou mira-image-prompt; para ANIMAR a imagem use mira-img-animator; este só POSICIONA uma imagem pronta. Use SEMPRE que o usuário disser "/mira-image", "coloca essa imagem no slide", "põe essa foto no slide", "insere essa imagem", "adiciona essa figura ao deck", "slide com essa imagem", "mostra esse print no slide", ou fornecer um arquivo de imagem/URL pedindo para exibir num slide.
---
