---
name: mira-3d
description: Adiciona elementos VERDADEIRAMENTE 3D (profundidade real, rotação contínua, arrastar/zoom) ao canvas de um slide do Mira, num card limpo com o 3D maximizado. Escolhe sozinha entre CSS 3D puro (formas simples), Three.js procedural (esfera de partículas, rede de nós, low-poly) e glTF (quando há um .glb ou aceita buscar um modelo gratuito). Herda a Regra Zero do mira-animator (o 3D sempre gira sozinho, pausa no arrasto e retoma) e as regras transversais (pt-BR, sem travessão, título sem ícone e com no máximo 6 palavras). Atenção: slide com .glb local não abre por file:// (o navegador bloqueia o fetch); nesse caso a skill sobe um servidor HTTP local e devolve o link. Use SEMPRE que o usuário disser "/mira-3d", "elemento 3D", "modelo 3D", "objeto 3D no slide", "esfera de partículas", "rede de nós em 3D", "carrega esse .glb", "modelo glTF", ou pedir um slide com profundidade real e rotação.
---
