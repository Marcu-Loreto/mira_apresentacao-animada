---
name: mira-qrcode
description: Insere num slide do Mira (novo ou existente) um QR code escaneável, grande e centralizado, gerado a partir de um link ou texto fornecido pelo usuário. O QR é gerado LOCALMENTE na criação do slide (pacote npm qrcode) e embutido como SVG inline, sem dependência em tempo de apresentação: funciona até por file://, sem API externa nem CDN. Card limpo no padrão do mira-3d: só o título do slide (sem ícone, máximo 6 palavras) e o QR grande no centro. Escaneabilidade manda no estilo: módulos escuros sobre cartão branco, zona de silêncio respeitada, laranja só na moldura. Herda a Regra Zero do mira-animator, mas o QR fica estático; o loop interno vai na moldura. Use SEMPRE que o usuário disser "/mira-qrcode", "QR code", "qrcode", "código QR", "gera um QR", "coloca um QR no slide", "QR do link", "QR de inscrição", "QR de CTA", "código para escanear", ou pedir um slide com um link escaneável.
---
